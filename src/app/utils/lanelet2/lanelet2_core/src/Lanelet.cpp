...
// junhee.lee add
ConstLineString3d LaneletData::calculatedCenterline() const {
  //!< @todo review this (thread safe?)
  auto centerline = std::atomic_load_explicit(&calculated_centerline_, std::memory_order_acquire);
  if (!centerline) {
    centerline = calculateCenterline(utils::to2D(leftBound()), utils::to2D(rightBound()));
    std::atomic_store_explicit(&calculated_centerline_, centerline, std::memory_order_release);
  }
  return *centerline;
}
